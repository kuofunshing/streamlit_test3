...
def llama2_chatbot_page():
    ...
    if not api_token:
        st.error("請設置Replicate API Token。")
        return

    user_input = st.text_input("用戶輸入：")
    if st.button("發送"):
        st.session_state['remaining_uses'] -= 1
        model = replicate.models.get("a16z-infra/llama13b-v2-chat")
        version = model.versions.get("b94a04e1e2de5b20a93f60e86f2ec3a23a0d7eeaf83c845a8059b89d1ceddb70")
        output = version.predict(prompt=user_input)
        st.write(f"Llama2 Chatbot: {output}")
